		...
		for {
			...
			case event, ok := <-watcher.Events:
				...
				if event.Op&(fsnotify.Write|fsnotify.Create|fsnotify.Rename) != 0 {
					logger.Debugf("File changed: %s", event)
...
func (w *Watcher) addToWatch(path string) {
	if err := w.watcher.Add(path); err != nil {
		logger.Warnf("Failed to watch %q (ignoring)", path)
	}
}

func (w *Watcher) removeFromWatch(path string) {
	if err := w.watcher.Remove(path); err != nil {
		logger.Warnf("Failed to remove watch %q (ignoring)", path)
	}
}
...
func (w *Watcher) onFileChanged(path string, onChanged func(key interface{})) {
	...
	if !ok {
		logger.Errorf("Unknown filesystem change: no mapping for %q", path)
		return
	}
